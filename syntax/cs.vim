...
" User Defined
syn match   csPropertyLabel             /\h\w*\ze[[:space:]\n]\+{[[:space:]\n]\+\(\(private\|public\|internal\|protected\)[[:space:]\n]\+\)\?get/
syn match   csCallingPropertyOrMember   /[.]\h\w*\>\((\)\@!/hs=s+1
syn match   csCallingMethod             /[.]\h\w*(/hs=s+1,he=e-1
syn match   csDeclaringVariableType     /\h\w*\ze[[:space:]\n]\+\h\w*[[:space:]\n]*\(=\|{\|,\|;\|)\)/
syn match   csUserType                  /[^.]\h\w*\ze\(,\|>\|(\)/ contained
syn match   csMethodNameDefinition      /\(\w\+\s\+\)\@<=\w\+(/he=e-1
syn match   csMethodTypeDefinition      /\<\h\w*\ze\s\+\h\w*(/
...
" Generics
syn match   csDeclaringVariableGenericType  /\h\w*</he=e-1 contained
syn region	csGenericBraces  start="\h\w*<" end=">" transparent oneline contains=csType,csUserType,csDeclaringVariableGenericType
",csUserInterface,csUserMethod
...
syn region	csNewType	start="@\@1<!\<new\>"hs=s+4 end="[;\n{(<\[]"me=e-1 contains=csNew,csUserType,csGenericBraces
...
syn cluster	csAll	contains=csCharacter,csClassType,csComment,csContextualStatement,csEndColon,csInterpolatedString,csIsType,csLabel,csLogicSymbols,csNewType,csConstant,csNumber,csOpSymbols,csOperatorError,csParens,csPreCondit,csRegion,csString,csSummary,csType,csMethodDefinition,csUnicodeNumber,csUnicodeSpecifier,csVerbatimString,csUserType,csUserIdentifier,csUserInterface,csUserMethod,csDeclaringVariableType,csDeclaringVariableGenericType,csGenericBraces
...
hi def link	csPropertyLabel	Keyword
hi def link	csCallingPropertyOrMember   csPropertyLabel
hi def link	csCallingMethod   Constant
hi def link	csDeclaringVariableType	Type
hi def link	csUserType	Type
hi def link	csMethodNameDefinition	Function
hi def link	csMethodTypeDefinition	Type
...
hi def link	csDeclaringVariableGenericType	csDeclaringVariableType
"hi def link	csGenericOwnerType	Type
